...
const PregTets = ({ onValuesPreg }) => {
  ...
  function evalPreg(val, p) {
    var puntaje = 0;
    if (val == dataAtencion[p].answerC) {
      puntaje = puntaje + 1;
    }
    return puntaje;
  }
  ...
  const handleChangeValue1 = (valor) => {
    ...
    if (valor != null) {
      setValue1(valor);
      var puntaje = evalPreg(valor, 0);
      onValuesPreg(puntaje);
    }
    return arrayD[0];
  };
